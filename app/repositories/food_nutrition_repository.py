...
def _get_es_doc_from_model(food_model: FoodNutritionModel) -> Dict[str, Any]:
    """SQLAlchemy 모델 객체를 Elasticsearch 문서 형태로 변환 (None 값 제외)"""
    doc = {
        "id": food_model.id,
    ...
    try:
        ...
        if es_client.ping():
            ...
            es_client.index(
                ...
                id=db_food_nutrition.id,
                body=doc_body,
                refresh="wait_for"
            )
            logger.info(f"FoodNutrition ID {db_food_nutrition.id}이(가) Elasticsearch에 인덱싱되었습니다.")
        else:
            logger.warning(f"ES Connection Error: FoodNutrition ID {db_food_nutrition.id} Elasticsearch 인덱싱 실패 (ping 실패).")
    except es_exceptions.ConnectionError as e:
        logger.error(f"ES Connection Error: FoodNutrition ID {db_food_nutrition.id} Elasticsearch 인덱싱 중 연결 오류: {e}")
    except Exception as e:
        logger.error(f"ES Error: FoodNutrition ID {db_food_nutrition.id} Elasticsearch 인덱싱 중 오류: {e}")
    ...
    if db_food_nutrition:
        ...
        try:
            ...
            if es_client.ping():
                ...
                logger.info(f"FoodNutrition ID {db_food_nutrition.id}이(가) Elasticsearch에서 업데이트되었습니다.")
            else:
                logger.warning(f"ES Connection Error: FoodNutrition ID {db_food_nutrition.id} Elasticsearch 업데이트 실패 (ping 실패).")
        except es_exceptions.ConnectionError as e:
            logger.error(f"ES Connection Error: FoodNutrition ID {db_food_nutrition.id} Elasticsearch 업데이트 중 연결 오류: {e}")
        except Exception as e:
            logger.error(f"ES Error: FoodNutrition ID {db_food_nutrition.id} Elasticsearch 업데이트 중 오류: {e}")
...
def delete_food_nutrition(db: Session, food_nutrition_id: int) -> Optional[FoodNutritionModel]:
    db_food_nutrition = db.query(FoodNutritionModel).filter(FoodNutritionModel.id == food_nutrition_id).first()
    if db_food_nutrition:
        deleted_item_id = db_food_nutrition.id 
        ...
        try:
            ...
            if es_client.ping():
                ...
                logger.info(f"FoodNutrition ID {deleted_item_id}이(가) Elasticsearch에서 삭제되었습니다.")
            else:
                logger.warning(f"ES Connection Error: FoodNutrition ID {deleted_item_id} Elasticsearch 삭제 실패 (ping 실패).")
        except es_exceptions.NotFoundError:
            logger.warning(f"ES Warning: FoodNutrition ID {deleted_item_id} (은)는 Elasticsearch 인덱스에 존재하지 않아 삭제할 수 없습니다.")
        except es_exceptions.ConnectionError as e:
            logger.error(f"ES Connection Error: FoodNutrition ID {deleted_item_id} Elasticsearch 삭제 중 연결 오류: {e}")
        except Exception as e:
            logger.error(f"ES Error: FoodNutrition ID {deleted_item_id} Elasticsearch 삭제 중 오류: {e}")
